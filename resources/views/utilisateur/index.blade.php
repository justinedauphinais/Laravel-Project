<!--/*****************************************************************************
 ...
 Fonctionnalité : Index des utilisateurs pour les consulter, suspendre et supprimer
 Date : 28 avril 2023
*****************************************************************************/-->
...
<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <form method="get" action="{{ route('indexUtilisateur') }}"
                    class="p-2 drop-shadow-boite border-2 border-dark-red bg-white shadow sm:rounded-lg">
            @csrf
                <h1 class="text-3xl font-fjalla font-bold text-center w-full mb-3">{{__('Liste des utilisateurs')}}</h1>
                    ...
                    <div>
                        <div class="mb-5 ">
                            ...
                            <select name="ville" id="ville" class="font-fjalla py-0 h-6 rounded-md border-2 border-black" >
                                <option value="" disabled selected>{{__('Filtrer par ville')}}</option>
            ...
            @else
                @foreach ($utilisateurs as $utilisateur)
                    <div id="div-{{ $utilisateur->id }}" class="p-2 drop-shadow-boite border-2 border-dark-red bg-white shadow sm:rounded-lg flex justify-around <?php echo ($utilisateur->est_actif == 1) ? "bg-white hover:bg-gray-100" : "bg-red-200 hover:bg-red-300" ?>">
                        <div>
                            {{ __('ID') }}
                        ...
                        <a href={{ route('showUtilisateur', ['id' => $utilisateur->id]) }} class="w-96">
                            <div class="my-1 flex">
                                <p class="font-bold font-fjalla w-40 text-right mr-1">{{__('Nom complet : ')}}</p>
                                <p class="font-fjalla">{{ $utilisateur->prenom . " " . $utilisateur->nom }}</p>
                            </div>
                            <div class="my-1 flex">
                                <span class="font-bold font-fjalla w-40 text-right mr-1">{{__('Courriel : ')}}</span>
                                <span class="font-fjalla">{{ $utilisateur->email }}</span>
                            </div>
                        </a>
                        <a href={{ route('showUtilisateur', ['id' => $utilisateur->id]) }} class="border-l-4 border-dark-red pl-2 w-96">
                            <div class="my-1 flex">
                            ...
                            <div class="my-1 flex">
                                ...
                                <span class="font-fjalla">{{ $utilisateur->numero_telephone }}</span>
                            </div>
                        </a>
                        <div class="border-l-4 border-dark-red pl-2 flex flex-col justify-between">
                            ...
                            <div>
                                <button id={{ $utilisateur->id }} class="suspendre bg-pale-yellow hover:bg-amber-100 font-fjalla font-bold border border-black rounded-md w-32 uppercase py-1 mr-1 my-1">
                                    @if ($utilisateur->est_actif == 1)
                                        {{ __('Suspendre le compte') }}
                                    @else
                                        {{ __('Réactiver le compte') }}
                                    @endif
                                </button>
                                <button id={{ $utilisateur->id }} class="supprimer bg-dark-yellow hover:bg-amber-400 font-fjalla font-bold border border-black rounded-md w-32 uppercase py-1 mr-1 my-1">
