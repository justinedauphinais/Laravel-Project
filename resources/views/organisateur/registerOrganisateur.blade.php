<!--/*****************************************************************************
 Fichier : registerOrganisateur.blade.php
 Auteur : Justine Dauphinais
 Fonctionnalité : Inscription d'un nouvel organisateur
 Date : 26 avril 2023
*****************************************************************************/-->
...
<x-app-layout>
    <div class="mt-10 mb-5 w-full flex flex-col sm:justify-center items-center">
        <div class="w-full sm:w-6/12 px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-lg mx-auto">
            <form method="POST" action="{{ route('registerOrganisateur') }}">
                @csrf

                <h1 class="font-fjalla text-2xl text-center">{{__('Inscription organisateur')}}</h1>
                ...
                <a href={{ url('/register/utilisateur') }} class="block font-fjalla text-right mt-2 underline hover:text-pale-blue">{{ __("Inscription utilisateur") }}</a>
            </form>
        </div>
    </div>
</x-app-layout>
